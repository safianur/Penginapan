                      ...
                      <tbody>
                        ...
                        @foreach ($user as $datauser)
                          <tr>
                            ...
                            <td>{{ $datauser->lokasi }} - {{ $datauser->nm_bisnis }}</td>
                            <td>{{ $datauser->jabatan }}</td>
                            <td><button type="button" class="btn btn-inverse-dark btn-rounded btn-icon" data-toggle="modal" data-target="#edituser{{ $datauser->id_user }}"><i class="ti-pencil-alt"></i></button></td>
                            <td><button type="button" class="btn btn-inverse-danger btn-rounded btn-icon" data-toggle="modal" data-target="#hapususer{{ $datauser->id_user }}"><i class="ti-trash"></i></button></td>
                          </tr>
                        @endforeach
                      </tbody>
                    </table>
              ...
              <form action="/pegawai/create" method="post" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                  <div class="form-group">
                    ...
                    <input type="text" class="form-control" name="nm_user" id="nm_user" placeholder="nama user">
                  </div>
                  <div class="form-group">
          ...
          <div class="modal" id="edituser{{ $dataedit->id_user }}" tabindex="-1" role="dialog">
            <div class="modal-dialog" role="document">
              <div class="modal-content">
                ...
                <form action="/user/update/{{ $dataedit->id_user }}" method="post" enctype="multipart/form-data">
                  @csrf
                  <div class="modal-body">
                    <div class="form-group">
                      <label for="nm_user">Nama</label>
                      <input type="text" class="form-control" name="nm_user" id="nm_user" placeholder="nama user" value="{{ $dataedit->nm_user }}" >
                    </div>
                    <div class="form-group">
                      <label for="kontak">No. Hp</label>
                      <input type="number" class="form-control" name="kontak" id="kontak" placeholder="no telepon yang aktif" value="{{ $dataedit->kontak }}">
                      <!-- error message untuk kontak -->
                      <div class="text-danger" style="font-size: 10px">
                        @error('kontak')
                            {{ $message }}
                    ...
                    <div class="form-group">
                      <label for="password">Password</label>
                      <input type="password" class="form-control" name="password" id="password" placeholder="password" value="{{ $dataedit->password }}">
                      <!-- error message untuk password -->
                      <div class="text-danger" style="font-size: 10px">
                        @error('password')
                            {{ $message }}
        ...
        @foreach ($user as $datahapus)
          <div class="modal" id="hapususer{{ $datahapus->id_user }}">
            <div class="modal-dialog">
              <div class="modal-content">
                <div class="modal-header">
                  <h4 class="modal-title">{{ $datahapus->nm_user }}</h4>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                  </button>
                </div>
                ...
                <div class="modal-footer">
                  ...
                  <a href="/user/delete/{{ $datahapus->id_user }}" class="btn btn-primary">Iya</a>
                </div>
              </div>
            </div>
          </div>
